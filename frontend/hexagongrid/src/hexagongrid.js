...
// // อันที่ chat แก้ให้
// const getGridDimensions = (columns, rows, N) => {
//   const hexSize = Math.floor(columns / (3 * columns + 0.5));
//   return {
//     columns,
//     hexSize,
//     hexWidth: hexSize * 2,
...
const HexagonGrid = (props) => {
  ...
  useEffect(() => {
    if (!isEmpty(hexagons) && gridWidth > 0 && gridHeight > 0) {
      setState((prevState) => {
        const newColumns =
          props.columns ||
          getGridDimensions(gridWidth, gridHeight, hexagons.length).columns;
        const newRows = props.rows || Math.ceil(hexagons.length / newColumns);
        return {
          ...getGridDimensions(
            gridWidth,
            gridHeight,
  ...
  }, [hexagons, gridWidth, gridHeight, props.columns, props.rows]);
  ...
  //         const newColumns =
  //           props.columns ||
  //           getGridDimensions(props.gridWidth, props.gridHeight, hexagons.length)
  //             .columns;
  //         const newRows = props.rows || Math.ceil(hexagons.length / newColumns);
  //         return {
  //           ...getGridDimensions(newColumns, newRows, hexagons.length),
  //         };
  //       });
  //     }
  //   }, [hexagons, props.gridWidth, props.gridHeight, props.columns, props.rows]);
